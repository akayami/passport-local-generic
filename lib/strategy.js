...
function Strategy(options, verify) {
	...
	this._fields = options.fields || ['username', 'password'];

	passport.Strategy.call(this);
	this.name = 'local-generic';
	this._verify = verify;
...
Strategy.prototype.authenticate = function (req, options) {
	...
	for(var x = 0; x < this._fields.length; x++) {
		//console.log(this._fields[x]);
		//var field = this._fields[x];
		var field = parseObjectNotatrion(this._fields[x]);
		//console.log(field)
		credentials[field] = req.body && req.body[field] || req.query && req.query[field];
